...
describe('PingModule', () => {
  ...
  describe('Controller', () => {
    ...
    it('PingController.firstPing avec aucun ping existant', async () => {
      await expect(pingController.firstPing()).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('PingController.createPing avec ping qui exist deja', async () => {
      ...
      await expect(pingController.createPing(createPingDto)).rejects.toThrow(
        ConflictException,
      );
    });
  });
  ...
  describe('Service', () => {
    ...
    it('PingService.getFirstPing avec aucun ping existant', async () => {
      await expect(pingService.getFirstPing()).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('PingService.createPing avec ping qui exist deja', async () => {
      ...
      await expect(pingService.createPing(createPingDto)).rejects.toThrow(
        ConflictException,
      );
    });
  });
