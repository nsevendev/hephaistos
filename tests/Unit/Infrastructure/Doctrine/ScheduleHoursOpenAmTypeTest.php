...
final class ScheduleHoursOpenAmTypeTest extends HephUnitTestCase
{
    private ScheduleHoursOpenAmType $type;
    private MySQLPlatform $platform;

    protected function setUp(): void
    {
        if (!Type::hasType('app_schedule_hours_open_am')) {
            Type::addType('app_schedule_hours_open_am', ScheduleHoursOpenAmType::class);
        }

        $this->type = Type::getType('app_schedule_hours_open_am');
        $this->platform = new MySQLPlatform();
    }

    public function testGetName(): void
    {
        self::assertSame('app_schedule_hours_open_am', $this->type->getName());
    }
    ...
    {
        $scheduleHoursOpenAm = $this->type->convertToPHPValue('Hello, World!', $this->platform);
        self::assertInstanceOf(ScheduleHoursOpenAm::class, $scheduleHoursOpenAm);
        self::assertSame('Hello, World!', $scheduleHoursOpenAm->value());
    }
    ...
    {
        $scheduleHoursOpenAm = $this->type->convertToPHPValue(null, $this->platform);
        self::assertNull($scheduleHoursOpenAm);
    }
     ...
     * @throws ConversionException
     */
    public function testConvertToDatabaseValueWithValidScheduleHoursOpenAm(): void
    {
        $scheduleHoursOpenAm = ScheduleHoursOpenAm::fromValue('Hello, Database!');
        $dbValue = $this->type->convertToDatabaseValue($scheduleHoursOpenAm, $this->platform);
